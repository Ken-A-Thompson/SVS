...
def open_output_files(K_adapt, n_adapt, B_adapt, u_adapt, alpha_adapt, sigma_adapt, theta_adapt, sigma_opt_adapt, maxgen_adapt, nfounders, opt1, style, style2, rep):
	"""
	This function opens the output files and returns file
	handles to each.
	"""

	sim_id = 'K%d_n%d_B%d_u%r_alpha%r_sigma%r_theta%r_sigmaopt%r_gens%d_founders%d_opt%s_adapt_%s_%s_rep%d' %(K_adapt, n_adapt, B_adapt, u_adapt, alpha_adapt, sigma_adapt, theta_adapt, sigma_opt_adapt, maxgen_adapt, nfounders,'-'.join(str(e) for e in opt1), style, style2, rep)
	data_dir = 'data'
...
# style2 = 'burn' #ancestor from burn-in
style2 = 'art' #artificially created ancestor

######################################################################
##WHICH ANCESTOR TO START FROM (if using real burn-in)##
######################################################################

if style == 'both' or 'sgv':

	K = K_adapt #number of individuals (positive integer)
	n = n_adapt #phenotypic dimensions (positive integer)
...
def main():
	...
	while rep < nReps + 1:
		...
		# open output files
		fileHandles = open_output_files(K_adapt, n_adapt, B_adapt, u_adapt, alpha_adapt, sigma_adapt, theta_adapt, sigma_opt_adapt, maxgen_adapt, nfounders, opt1, style, style2, rep) 
		...
		while gen < maxgen_adapt + 1:
			...
			off = np.repeat(np.append(off1, off2, axis=0), B_adapt, axis=0) #each product of meiosis produced B times
			...
			whomuts = np.where(rand3 < u_adapt) #indices of mutants
