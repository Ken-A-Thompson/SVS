...
def open_output_files(n, N, alpha, u, sigma, data_dir):
	...
	outfile_A = open("%s/MainText_MoreParameters_N0.1K_%s.csv" %(data_dir, sim_id), "w") #summary data
	outfile_B = open("%s/MainText_MoreParameters_N0.1K_phenotypes_%s.csv" %(data_dir, sim_id),"wb") #hybrid phenotypes
	# outfile_C = open("%s/MainText_MoreParameters_N0.1K_ancestral_mutations_%s.csv" %(data_dir, sim_id),"wb") #stats on ancestral mutations
...
n_angles = 50 #number of angles between optima to simulate (including 0 and 180) (>=2)

n_mut_list = [[0, 100], [0, 100], [0, 100]] # de novo and one SGV scenario
				...
				while j < len(theta2_list):
					...
					while i < len(n_mut_list[l]):
						...
						while rep < nreps:
							...
							print('N=%d, sigma=%.2f, n=%d, angle=%r, rep=%d, n_muts=%d' %(N_adapt, sigma_adapt, n, round(angles[j]*180/math.pi,2), rep+1, n_muts))
