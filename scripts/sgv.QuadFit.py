...
def open_output_files(n, N, alpha, u, sigma, data_dir):
	...
	outfile_A = open("%s/QuadFit_%s.csv" %(data_dir, sim_id), "w") #summary data
	outfile_B = open("%s/QuadFit_phenotypes_%s.csv" %(data_dir, sim_id),"wb") #hybrid phenotypes
	outfile_C = open("%s/QuadFit_ancestral_mutations_%s.csv" %(data_dir, sim_id),"wb") #stats on ancestral mutations
...
def fitness(phenos, theta, sigma):
     ...
     w = 1 - (sigma/2) * dist**2 # fitness (quadratic)
     return w * (w>0.0001)
