...
def open_output_files(n, N, alpha, u, sigma, data_dir):
	...
	outfile_A = open("%s/LinFit_%s.csv" %(data_dir, sim_id), "w") #summary data
	outfile_B = open("%s/LinFit_phenotypes_%s.csv" %(data_dir, sim_id),"wb") #hybrid phenotypes
	outfile_C = open("%s/LinFit_ancestral_mutations_%s.csv" %(data_dir, sim_id),"wb") #stats on ancestral mutations
