...
alpha = 0.5 #mutational sd (positive real number)
...
opt_dists = list(np.arange(0.1, 1.15, 0.1)) #distances to optima
		...
		while j < len(opt_dists):
			...
			while i < len(n_mut_list):
				...
				while rep < nreps:
					...
					#save data
					write_data_to_output(fileHandles, [theta1, theta2, rep+1, n_mut_list[i], off_sd, opt_dists[j], ['parallel','divergent'][k]])
