...
def open_output_files(n, N, alpha, u, sigma, data_dir):
	...
	outfile_A = open("%s/MainText_MoreParameters_N1K_%s.csv" %(data_dir, sim_id), "w") #summary data
	outfile_B = open("%s/MainText_MoreParameters_N1K_phenotypes_%s.csv" %(data_dir, sim_id),"wb") #hybrid phenotypes
	outfile_C = open("%s/MainText_MoreParameters_N1K_ancestral_mutations_%s.csv" %(data_dir, sim_id),"wb") #stats on ancestral mutations
