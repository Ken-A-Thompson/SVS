					...
					while j < len(opt_dists):
						...
						while i < len(n_mut_list[l]):
							...
							while rep < nreps:
								...
								n1 = sum(p == 0) + sum(p == 1) #number of fixed loci in pop1
								n2 = sum(q == 0) + sum(q == 1) #number of fixed loci in pop2
								r = (p[len(mutfound)-n_muts:len(mutfound)] + q[len(mutfound)-n_muts:len(mutfound)]) / 2 #average allele frequency across the two populations for all shared loci that were initially segregating
								n12 = sum(r == 0) + sum(r == 1) #number of loci that have fixed in both populations
