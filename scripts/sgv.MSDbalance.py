...
def open_output_files(n, K, alpha, B, u, data_dir):
	...
	outfile_A = open("%s/MSDbalance_%s.csv" %(data_dir, sim_id), "w")
	return outfile_A
...
theta_list = np.array([[0,0]]) #optimum phenotypes for population
				...
				while gen < maxgen + 1:
					...
					if gen % gen_rec == 0:
						...
						for x in range(len(w)):
							write_data_to_output(fileHandles, [gen] + [s[x]]) #gen, phenotype in each dimension, fitness
