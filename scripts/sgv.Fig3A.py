		...
		while j < len(theta2_list):
			...
			while i < len(n_mut_list[l]):
				...
				while rep < nreps:
					...
					#print an update
					print('n=%d, angle=%r, rep=%d, n_muts=%d, segreagation variance=%.3f, distance between optima=%.3f' %(n, round(angles[j]*180/math.pi,2), rep+1, n_muts, segvar, opt_dist * (2*(1-math.cos(angles[j])))**(0.5))) 
