...
def open_output_files(n, N, alpha, u, sigma, data_dir):
	...
	outfile_A = open("%s/FigSE_%s.csv" %(data_dir, sim_id), "w")
	return outfile_A
...
n_mut_list = [list(np.arange(0, 141, 1))]
# n_mut_list = [list(np.arange(0, 151, 1)), list(np.arange(0, 151, 1)), list(np.arange(0, 151, 1))] #starting nmuts, final n_muts, interval
...
maxgen = 0 #total number of generations population adapts for
