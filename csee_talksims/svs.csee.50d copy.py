...
n = 20 #phenotypic dimensions (positive integer >=1)
data_dir = 'csee_sims_data'
...
n_mut_list = [700] #number of mutations (positive integer >=1)
	...
	while j < len(theta2_list):
		...
		while i < len(n_mut_list):
			...
			while rep < nreps:
				...
				#print an update
				print('angle=%r, rep=%d, n_muts=%d, seg var=%.3f, distance between optima=%.3f' %(round(angles[j]*180/math.pi,2), rep+1, n_muts, segvar, opt_dist * (2*(1-math.cos(angles[j])))**(0.5))) 
				
				#save data
				write_data_to_output(fileHandles, [round(angles[j]*180/math.pi,2), rep+1, n_muts, segvar, opt_dist * (2*(1-math.cos(angles[j])))**(0.5)])
