...
for i in range(len(opt1s)):
    ...
    # sim_id = 'K%d_n%d_B%d_u%r_alpha%r_gens%r_founders%d_opt%s_adapt_SGV' %(Kadapt,n,B,u,alpha,maxgen,nfounders,'-'.join(str(e) for e in opt1s[i]))
    sim_id = 'K%d_n%d_B%d_u%r_alpha%r_gens%r_founders%d_opt%s_adapt_DNM' %(Kadapt,n,B,u,alpha,maxgen,nfounders,'-'.join(str(e) for e in opt1s[i]))
